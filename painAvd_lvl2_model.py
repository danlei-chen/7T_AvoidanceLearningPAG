...
# subject_list = ['sub-018','sub-019','sub-020','sub-024','sub-025','sub-026','sub-031','sub-032','sub-033','sub-041','sub-055','sub-056','sub-058','sub-059','sub-062','sub-064','sub-065','sub-067''sub-070','sub-072','sub-080','sub-088','sub-090','sub-091']
subject_list = ['sub-018','sub-020','sub-025','sub-026','sub-032','sub-041','sub-059','sub-062','sub-064','sub-065','sub-067','sub-072','sub-080','sub-088','sub-090','sub-091']
...
if not os.path.exists('/scratch/data/smooth/combined_copes'):
    os.mkdir('/scratch/data/smooth/combined_copes')
if os.environ['FWHM'] == '5':
    all_cope_files = sorted(glob.glob('/scratch/data/smooth/sub-*/model/fwhm_5.0_sub-*/_modelestimate0/cope*nii.gz'))
    if not os.path.exists('/scratch/data/smooth/combined_copes/5.0'):
        os.mkdir('/scratch/data/smooth/combined_copes/5.0')
    base_dir = '/scratch/data/smooth/combined_copes/5.0'
elif os.environ['FWHM'] == '1.5':
    all_cope_files = sorted(glob.glob('/scratch/data/smooth/sub-*/model/fwhm_1.5_sub-*/_modelestimate0/cope*nii.gz'))
    if not os.path.exists('/scratch/data/smooth/combined_copes/1.5'):
        os.mkdir('/scratch/data/smooth/combined_copes/1.5')
    base_dir = '/scratch/data/smooth/combined_copes/1.5'
elif os.environ['FWHM'] == 'nosmooth':
    all_cope_files = sorted(glob.glob('/scratch/data/smooth/sub-*/model/fwhm_nosmooth_sub-*/_modelestimate0/cope*nii.gz'))
    if not os.path.exists('/scratch/data/smooth/combined_copes/nosmooth'):
        os.mkdir('/scratch/data/smooth/combined_copes/nosmooth')
    base_dir = '/scratch/data/smooth/combined_copes/nosmooth'

for x, subj_name in enumerate(subject_list):
    print(subj_name)
    sub_files = [i for i in all_cope_files if subj_name in i] 
    if not os.path.exists(os.path.join(base_dir, subj_name)):
        os.mkdir(os.path.join(base_dir, subj_name))
    
    combined_cope_data = np.empty(len(list(full_cons.keys())), dtype=object)
    for y, cope_name in enumerate(list(full_cons.keys())):
        print(cope_name)
        cope_files = [i for i in sub_files if cope_name in i] 

        all_runs_files = np.empty(len(cope_files), dtype=object)
        for z, run_name in enumerate(cope_files):
            run_file = nib.load(cope_files[z])
            run_file_data = run_file.get_data()
            all_runs_files[z]=run_file_data
        all_runs_files = np.stack([i for i in all_runs_files],axis=3)
        combined_cope_data[y] = np.mean(all_runs_files, axis=3)

        ref = nib.load(cope_files[0])
        ref.header['cal_max'] = np.max(combined_cope_data[y]) # adjust min and max header info.
        ref.header['cal_min'] = np.min(combined_cope_data[y])
        out_img = nib.Nifti1Image(combined_cope_data[y], ref.affine, ref.header)
        nib.save(out_img, os.path.join(base_dir, subj_name, cope_name + '.nii'))
#############################################################################
...
#     exit()
lvl2_tfce_wf.inputs.inputspec.copes_template = os.path.join(base_dir,'sub-*/cope*.nii')
...
lvl2_tfce_wf.inputs.inputspec.sinker_subs = [
    ...
    ('contrast_cope2', 'contrast_pain_US'),
    ('contrast_cope3', 'contrast_pain_USnegneu'),
    ('contrast_cope4', 'contrast_pain_USPE')
]
