...
subjCommonEventFile_data_dir = '/scratch/data/sub-*/func/'
commonEventFile = '*_task-pain2_events.tsv'
subj_list = glob.glob(subjCommonEventFile_data_dir)
for subj_dir in subj_list:

    #read common event files
    subj_commonFile_dir = glob.glob(os.path.join(subj_dir,commonEventFile))[0] #[0] b/c there's only one emo2 file
    subj_df_original = pd.read_table(subj_commonFile_dir, sep='\t')
    if 'pain' in subj_commonFile_dir:
        subj_ID = subj_commonFile_dir[-29:-22]
    elif 'emo' in subj_commonFile_dir:
        subj_ID = subj_commonFile_dir[-28:-21]
        
    #make a new column on original df
    subj_df_original['expRating']='NaN'
    subj_df_original['expRating'][list(subj_df_original['trial_type']=='CS')]=np.array(subj_df_original['ratingResp'][subj_df_original['trial_type']=='expRating'])
    subj_df_original['unpRating']='NaN'
    subj_df_original['unpRating'][list(subj_df_original['trial_type']=='US')]=np.array(subj_df_original['ratingResp'][subj_df_original['trial_type']=='unpRating'])

    #event file to model CS and US  
    subj_df_event = subj_df_original[(subj_df_original['trial_type'] == 'CS') | (subj_df_original['trial_type'] == 'US') | (subj_df_original['trial_type'] == 'expRating') | (subj_df_original['trial_type'] == 'unpRating')]
    subj_df_event['trial_type'][subj_df_original['trial_type'] == 'expRating'] = 'rating'
    subj_df_event['trial_type'][subj_df_original['trial_type'] == 'unpRating'] = 'rating'

    subj_df_event['Neg_Neu'] = 'NaN'
    subj_df_event['Neg_Neu'][subj_df_event['US_trial_type']=='negative']=-1
    subj_df_event['Neg_Neu'][subj_df_event['US_trial_type']=='neutral']=1

    subj_df_event['PE'] = 'NaN'
    subj_df_event['PE'][subj_df_event['trial_type']=='US'] = np.array(subj_df_original['ratingResp'][subj_df_original['trial_type']=='unpRating'])-np.array(subj_df_original['ratingResp'][subj_df_original['trial_type']=='expRating'])
    ...
    subj_df_event.to_csv(os.path.join(subj_dir,subj_ID+'_task-pain2_events_Exp-NN-PE.tsv'),index=True,sep='\t')
...
model_wf.inputs.inputspec.conditions = ['CS', ['CS','expRating', 'cent', 'norm'],'rating', 'US', ['US','Neg_Neu', 'no_cent', 'no_norm'], ['US','PE', 'cent', 'norm']] # parameter to model from task file.
model_wf.inputs.inputspec.contrasts = [['CS', 'T', ['CS'], [1]],   
                                       ['CS_expRating', 'T', ['CS_expRating'], [1]], 
                                       ['rating', 'T', ['rating'], [1]],                                
                                       ['US', 'T', ['US'], [1]],
                                       ['US_Neg_Neu', 'T', ['US_Neg_Neu'], [1]],
                                       ['US_PE', 'T', ['US_PE'], [1]]]
...
model_wf.inputs.inputspec.bold_template = {'bold': '/scratch/data/sub-*/func/sub-*_task-pain2_bold_space-MNI152NLin2009cAsym_preproc.nii.gz'}
model_wf.inputs.inputspec.mask_template = {'mask': '/scratch/data/sub-*/func/sub-*_task-pain2_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz'}
model_wf.inputs.inputspec.task_template = {'task': '/scratch/data/sub-*/func/sub-*_task-pain2_events_Exp-NN-PE.tsv'}
model_wf.inputs.inputspec.confound_template = {'confound': '/scratch/data/sub-*/func/sub-*_task-pain2_bold_confounds.tsv'} 
...
model_wf.inputs.inputspec.proj_name = 'painTest_noTask'
...
full_model_wf.base_dir = '/scratch/data/painTest_noTask'
...
########## RUN ##################################################
full_model_wf.run(plugin='MultiProc', plugin_args={'n_procs': 1})
