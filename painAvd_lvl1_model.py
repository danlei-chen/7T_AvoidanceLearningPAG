...
commonEventFile = '*_task-pain3_run-*_events.tsv'
...
for subj_dir in subj_list:
    ...
    for run_dir in run_list:
        subj_commonFile_dir = run_dir
        subj_df_original = pd.read_table(subj_commonFile_dir, sep='\t')
        if 'pain' in subj_commonFile_dir:
            subj_ID = subj_commonFile_dir[-36:-29]
        elif 'emo' in subj_commonFile_dir:
            subj_ID = subj_commonFile_dir[-35:-28]
        run_num = subj_commonFile_dir[-13:-11]
        ...
        subj_df_event['trial_type'][subj_df_event['trial_type'] == 'CS'] = ['CS_combo'] * len(subj_df_event['trial_type'][subj_df_event['trial_type'] == 'CS'])
        subj_df_event['duration'][subj_df_event['trial_type'] == 'CS_combo'] = [x+y for x, y in zip(subj_df_event['duration'][subj_df_event['trial_type'] == 'CS_combo'].tolist(),subj_df_event['duration'][subj_df_event['trial_type'] == 'decision'].tolist())]
        subj_df_event['US_trialType'][subj_df_event['US_trialType'] == 'neutral'] = [0] * len(subj_df_event['US_trialType'][subj_df_event['US_trialType'] == 'neutral'])
        subj_df_event['US_trialType'][subj_df_event['US_trialType'] == 'negative'] = [1] * len(subj_df_event['US_trialType'][subj_df_event['US_trialType'] == 'negative'])
        subj_df_event = subj_df_event[subj_df_event['trial_type'] != 'decision'] 
        ...
        subj_df_event.to_csv(os.path.join(subj_dir,subj_ID+'_task-pain3_run-'+run_num+'_events_modeledPE.tsv'),index=True,sep='\t')
...
model_wf.inputs.inputspec.conditions = ['CS_combo', 'US', ['US','US_trialType', 'no_cent', 'no_norm'], ['US','PE', 'cent', 'norm']] # parameter to model from task file.
model_wf.inputs.inputspec.contrasts = [['CS_combo', 'T', ['CS_combo'], [1]],                                 
                                       ['US', 'T', ['US'], [1]],
                                       ['US_US_trialType', 'T', ['US_US_trialType'], [1]],
                                       ['US_PE', 'T', ['US_PE'], [1]]]
...
model_wf.inputs.inputspec.bold_template = {'bold': '/scratch/data/sub-*/func/sub-*_task-pain3_run-*_bold_space-MNI152NLin2009cAsym_preproc.nii.gz'}
model_wf.inputs.inputspec.mask_template = {'mask': '/scratch/data/sub-*/func/sub-*_task-pain3_run-*_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz'}
model_wf.inputs.inputspec.task_template = {'task': '/scratch/data/sub-*/func/sub-*_task-pain3_run-*_events_modeledPE.tsv'}
model_wf.inputs.inputspec.confound_template = {'confound': '/scratch/data/sub-*/func/sub-*_task-pain3_run-*_bold_confounds.tsv'} 
...
model_wf.inputs.inputspec.proj_name = 'painAvd_modeledPE'
...
subject_list = [sys.argv[1]+'_task-pain3_run-01',
                sys.argv[1]+'_task-pain3_run-02',
                sys.argv[1]+'_task-pain3_run-03',
                sys.argv[1]+'_task-pain3_run-04',
                sys.argv[1]+'_task-pain3_run-05']
fwhm_list = [5.,1.5]
...
full_model_wf.base_dir = '/scratch/data/painAvd_modeledPE'
...
########## RUN ##################################################
full_model_wf.run(plugin='MultiProc', plugin_args={'n_procs': 3})
